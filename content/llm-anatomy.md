...
Modern language models usually use *subword tokenisation* as a compromise.
...
The actual split depends on the tokeniser and on the data from which its
vocabulary was constructed.

:::{admonition} Tokenisation is part of the model
:class: important
...
Tokenisation affects the length of the sequence seen by the model. This in
...
tokeniser may represent frequent English words efficiently while dividing a
...
Without using a real tokeniser, propose a possible subword vocabulary that
...
sequences. Real tokenisers choose a compromise based on frequencies in their
training corpus, often using an algorithm called byte-pair encoding ([BPE](https://en.wikipedia.org/wiki/Byte-pair_encoding)).
...
2. a position-wise feed-forward network, often called an MLP (multi-layer
   perceptron).
...
feed-forward layers, residual connections, and normalization. [Source](https://en.wikipedia.org/wiki/Transformer_(deep_learning)#/media/File:Transformer,_full_architecture.png)
```
...
Here, {math}`X` contains the input representations for all positions. The matrices
{math}`W_Q`, {math}`W_K`, and {math}`W_V` are learned model parameters.
...
another head always connects adjectives to nouns, but while it is a useful
...
matrix is an important reason why long contexts can be expensive. There are a
few tricks to mitigate this, such as [KV
caching](https://huggingface.co/blog/not-lain/kv-caching).
